        ...
        <>
            ...
            <Stack spacing={1} alignItems="flex-start" justifyContent="flex-start" ml={4} mt={2}>
                {menuList.map((item, index) => {
                    return (
                        <Link to={item.path} key={index}><Button
                            color="primary"
                            startIcon={item.icon}
                            size="large"
                            variant="text"
                        ><Typography variant='h6' color="primary.main" ml={2}>{item.name}</Typography>
                        </Button>
                        </Link>
                    )
                })}
                <Typography variant="h6" sx={{ position: "absolute", bottom: "1rem" }} color="third.main">Copyright © 2023 VidyaVault, <br></br>Inc. All rights reserved.</Typography>
            </Stack>
        </>
    );
